...
*/
  ...
  public static void main(String[] args) throws Exception {
    ...
    QueryParser parser = new QueryParser("contents", new StandardAnalyzer());  // #1
    BooleanQuery.Builder builder = new BooleanQuery.Builder();

    builder.add(allBooks, BooleanClause.Occur.SHOULD);                             // #1
    builder.add(parser.parse("java OR action"), BooleanClause.Occur.SHOULD); // #1
    BooleanQuery query = builder.build();

    //Directory directory = TestUtil.getBookIndexDirectory();                     // #2

    System.out.println("index path: " + new File( "index").getCanonicalPath());
